...
def _do_plot_candle_html(date, p_open, high, low, close, symbol, save):
    ...
    p.segment(date.to_pydatetime(), high, date.to_pydatetime(), low, color="black")
    # noinspection PyUnresolvedReferences
    p.rect(date.to_pydatetime()[inc], mids[inc], w, spans[inc], fill_color=__colorup__, line_color=__colorup__)
    # noinspection PyUnresolvedReferences
    p.rect(date.to_pydatetime()[dec], mids[dec], w, spans[dec], fill_color=__colordown__, line_color=__colordown__)
